...
var LIVESTOCK_SPECIES = [
  ...
  { key:"sheep",   label:"Sheep",    icon:"\uD83D\uDC11", hasGenetics:true },
  { key:"goat",    label:"Goats",    icon:"\uD83D\uDC10", hasGenetics:false },
  { key:"pig",     label:"Pigs",     icon:"\uD83D\uDC16", hasGenetics:true },
  { key:"duck",    label:"Ducks",    icon:"\uD83E\uDD86", hasGenetics:true },
  { key:"chicken", label:"Chickens", icon:"\uD83D\uDC14", hasGenetics:true }
];
...
// Goat breed lists — goat-data.js defines these if loaded; these are fallbacks
if (typeof GOAT_DAIRY_BREEDS === "undefined") var GOAT_DAIRY_BREEDS = ["Nubian","Alpine","LaMancha","Saanen","Oberhasli","Toggenburg","Nigerian Dwarf","Sable","Guernsey"];
if (typeof GOAT_MEAT_BREEDS === "undefined") var GOAT_MEAT_BREEDS = ["Boer","Kiko","Spanish","Myotonic","Savanna","Rangeland","Arapawa","Texmaster"];
if (typeof GOAT_FIBER_BREEDS === "undefined") var GOAT_FIBER_BREEDS = ["Angora","Cashmere","Pygora","Nigora"];
if (typeof GOAT_DUAL_BREEDS === "undefined") var GOAT_DUAL_BREEDS = ["Pygmy","Kinder","Jamnapari","Damascus"];

if (typeof GOAT_MILK_OUTPUT === "undefined") var GOAT_MILK_OUTPUT = {
  dairy: 15,
  dual:  8,
  meat:  0,
  fiber: 0
...
if (typeof GOAT_MEAT_PRICE === "undefined") var GOAT_MEAT_PRICE = {
  dairy: 90,
  dual:  90,
  meat:  120,
  fiber: 80
...
if (typeof randomGoatType !== "function") {
...
if (typeof goatBreedForType !== "function") {
...
function generateLivestockStock() {
  ...
  LIVESTOCK_SPECIES.forEach(function(sp) {
    ...
    if (sp.key === "horse") {
      for (var i=0; i<qty; i++) {
        if (typeof createHorse === "function" && typeof HORSE_BREED_DEFS !== "undefined") {
          var sex = randomSex();
          var hBreed = HORSE_BREED_DEFS[Math.floor(Math.random()*HORSE_BREED_DEFS.length)];
          var ha = createHorse(hBreed.name, sex);
          ...
          var ha = { id:lsId(), type:type, breed:breed.name, species:"horse",
            ...
            ageMonths: Math.floor(Math.random()*48)+18 };
      ...
      for (var i=0; i<qty; i++) {
        ...
        var breed = breeds[Math.floor(Math.random()*breeds.length)];
        if (typeof createCattle === "function") {
          var sex = randomSex();
          var ca = createCattle(breed, cowType, sex);
          stock[sp.key].push(ca);
        } else {
          var ca = { id:lsId(), type:cowType, breed:breed, sex:randomSex(), price:0, species:"cow", ageMonths: Math.floor(Math.random()*48)+18 };
      ...
      for (var i=0; i<qty; i++) {
        ...
        if (typeof createGoat === "function") {
          var sex = randomSex();
          var ga = createGoat(goatBreed, goatType, sex);
          stock[sp.key].push(ga);
        } else {
          var ga = { id:lsId(), type:goatType, breed:goatBreed, sex:randomSex(), price:0, species:"goat", ageMonths: Math.floor(Math.random()*36)+8 };
          ga.price = (GOAT_MEAT_PRICE[goatType]||120) + (goatType==="dairy"||goatType==="dual" ? 30 : 0);
          stock[sp.key].push(ga);
        }
      }
    } else if (sp.key === "pig") {
      for (var i=0; i<qty; i++) {
        var pigType = typeof randomPigType === "function" ? randomPigType() : "commercial";
        var pigBreed = typeof pigBreedForType === "function" ? pigBreedForType(pigType) : "Yorkshire";
        if (typeof createPig === "function") {
          var sex = randomSex();
          var pa = createPig(pigBreed, pigType, sex);
          stock[sp.key].push(pa);
        } else {
          var pa = { id:lsId(), type:pigType, breed:pigBreed, sex:randomSex(), price:0, species:"pig", ageMonths: Math.floor(Math.random()*24)+6 };
          pa.price = 50;
          stock[sp.key].push(pa);
        }
      }
    } else if (sp.key === "sheep") {
      for (var i=0; i<qty; i++) {
        var sheepType = typeof randomSheepType === "function" ? randomSheepType() : "dual";
        var sheepBreed = typeof sheepBreedForType === "function" ? sheepBreedForType(sheepType) : "Merino";
        if (typeof createSheep === "function") {
          var sex = randomSex();
          var sa = createSheep(sheepBreed, sheepType, sex);
          stock[sp.key].push(sa);
        } else {
          var sa = { id:lsId(), type:sheepType, breed:sheepBreed, sex:randomSex(), price:0, species:"sheep", ageMonths: Math.floor(Math.random()*36)+8 };
          sa.price = 100;
          stock[sp.key].push(sa);
        }
      }
      ...
      for (var i=0; i<qty; i++) {
        var duckType = typeof randomDuckType === "function" ? randomDuckType() : "dual";
        var duckBreed = typeof duckBreedForType === "function" ? duckBreedForType(duckType) : "Pekin";
        if (typeof createDuck === "function") {
          var sex = randomSex();
          var da = createDuck(duckBreed, duckType, sex);
          stock[sp.key].push(da);
        } else {
          var da = { id:lsId(), type:duckType, breed:duckBreed, sex:randomSex(), price:0, species:"duck", ageMonths: Math.floor(Math.random()*18)+4 };
          da.price = 20;
          stock[sp.key].push(da);
        }
      }
    } else if (sp.key === "chicken") {
      for (var i=0; i<qty; i++) {
        var chickenType = typeof randomChickenType === "function" ? randomChickenType() : "dual";
        var chickenBreed = typeof chickenBreedForType === "function" ? chickenBreedForType(chickenType) : "Plymouth Rock";
        if (typeof createChicken === "function") {
          var sex = randomSex();
          var ca = createChicken(chickenBreed, chickenType, sex);
          stock[sp.key].push(ca);
        } else {
          var ca = { id:lsId(), type:chickenType, breed:chickenBreed, sex:randomSex(), price:0, species:"chicken", ageMonths: Math.floor(Math.random()*18)+4 };
          ca.price = 10;
          stock[sp.key].push(ca);
        }
      }
...
var LIVESTOCK_PRICES = {
  chicken: 5,
  duck:    function(a){ return ({egg:25,meat:30,dual:28,ornamental:50})[a.type]||20; },
  pig:     function(a){ return ({heritage:150,commercial:80,mini:200})[a.type]||100; },
  sheep:   function(a){ return ({wool:200,meat:150,dual:175})[a.type]||150; },
  goat:    function(a){ return ({dairy:90,meat:120,fiber:100,dual:90})[a.type]||90; },
  cow:     function(a){ return a.type==="dairy" ? 600 : 500; },
  horse:   function(a){ return (HORSE_GROUP_PRICES[(a.group||"Light")])||1200; }
};

var HORSE_GROUP_PRICES = {
  Racing:2000, Light:1200, Sport:2500, Western:900,
  Draft:1500, Gaited:1100, Pony:600
};

var HORSE_RESALE_RATE = 0.85;
var LIVESTOCK_RESALE_RATE = 0.50;
...
var MEAT_PRICES = {
  ...
  goat:  { label: "Chevon", price: 120, commodity: "chevon",
    ...
    fiber: { label: "Chevon (Fiber)", price: 80,  commodity: "chevon" }
  },
...
// ── LONGEVITY SYSTEM ─────────────────────────────────────────────────────────────────
var PRIME_AGE = { dog:96, horse:180, cow:84, goat:84, sheep:72, pig:48, chicken:24, duck:36 };
var MAX_LIFESPAN = { cow:240, goat:168, sheep:144, pig:96, chicken:72, duck:96 };
var DECLINE_WINDOW = 24;
var LOCK_COOLDOWN_MS = 24*60*60*1000;

function getDeclineMultiplier(m) {
  if (m<=0) return 1.0;
  if (m<=12) return Math.max(0, 1.0-(m*0.008));
  var b=1.0-(12*0.008); var s=m-12;
  return Math.max(0, b-(s*s*0.005));
}
function getProductionMultiplier(age,sp) {
  var p=PRIME_AGE[sp]; if(!p) return 1.0;
  var ds=p+DECLINE_WINDOW; if(age<ds) return 1.0;
  return getDeclineMultiplier(age-ds);
}
function isPastPrime(age,sp) { var p=PRIME_AGE[sp]; return p?age>=p:false; }
function isDeclining(age,sp) { var p=PRIME_AGE[sp]; return p?age>=(p+DECLINE_WINDOW):false; }
function canBreedByAge(age,sp) { var p=PRIME_AGE[sp]; return p?age<(p+DECLINE_WINDOW):true; }
function canShowByAge(age,sp) { var p=PRIME_AGE[sp]; return p?age<p:true; }
function canSlaughterByAge(age,sp) { return !isDeclining(age,sp); }
function isAnimalLocked(a) { return !!(a&&a.locked); }
function canLockAnimal(a) { if(!a||a.locked) return false; if(!a.lockedCooldownUntil) return true; return Date.now()>=a.lockedCooldownUntil; }
function getDeclinedScore(base,age,sp) { var m=getProductionMultiplier(age,sp); return m>=1.0?base:Math.max(1,Math.round(base*m)); }
...
var COMMODITY_PRICES = {
  ...
  mohair:       { label: "Mohair",      unit: "lb",   price: 8.00  }
};
...
function LivestockMarket(_ref) {
  ...
  var _e=_slicedToArray(useState({}),2),stock=_e[0],setStock=_e[1];
  var _f=_slicedToArray(useState(""),2),currentCycle=_f[0],setCurrentCycle=_f[1];
  ...
  var _sb=_slicedToArray(useState(""),2),selBreed=_sb[0],setSelBreed=_sb[1];
  var _sf=_slicedToArray(useState("all"),2),subFilter=_sf[0],setSubFilter=_sf[1];
  var _ld=_slicedToArray(useState(true),2),loading=_ld[0],setLoading=_ld[1];
  var _msg=_slicedToArray(useState(""),2),statusMsg=_msg[0],setStatusMsg=_msg[1];

  // Convert flat Supabase rows into species-keyed stock object
  function rowsToStock(rows) {
    var s = {};
    LIVESTOCK_SPECIES.forEach(function(sp){ s[sp.key]=[]; });
    (rows||[]).forEach(function(row){
      var animal = row.animal_data || {};
      animal.id = row.id; // use the market_stock row ID for buy/delete
      animal._marketId = row.id;
      var sp = row.species || animal.species || "";
      // Normalize market animals that lack genetics (from old market cycles)
      if (sp === "sheep" && !animal.genome && typeof normalizeSheep === "function") {
        animal = Object.assign(animal, normalizeSheep(animal));
      }
      if (sp === "goat" && !animal.genome && typeof normalizeGoat === "function") {
        animal = Object.assign(animal, normalizeGoat(animal));
      }
      if (sp === "pig" && !animal.genome && typeof normalizePig === "function") {
        animal = Object.assign(animal, normalizePig(animal));
      }
      if (sp === "duck" && !animal.genome && typeof normalizeDuck === "function") {
        animal = Object.assign(animal, normalizeDuck(animal));
      }
      if (sp === "chicken" && !animal.genome && typeof normalizeChicken === "function") {
        animal = Object.assign(animal, normalizeChicken(animal));
      }
      if (s[sp]) s[sp].push(animal);
    });
    return s;
  }

  // Load global market on mount
  function loadGlobalMarket() {
    if (!window.baSupabaseSync || !window.baSupabaseSync.fetchGlobalMarketStock) {
      // Fallback to local generation if Supabase sync not available
      console.log("[BA Market] Supabase sync not available, using local stock");
      setStock(generateLivestockStock());
      setLoading(false);
      return;
    }
    setLoading(true);
    setStatusMsg("Loading market...");
    window.baSupabaseSync.fetchGlobalMarketStock().then(function(result) {
      setCurrentCycle(result.cycleId);
      if (result.items.length > 0) {
        // Stock exists for this cycle
        setStock(rowsToStock(result.items));
        setLoading(false);
        setStatusMsg("");
      } else {
        // Empty cycle — this player generates and uploads
        setStatusMsg("Stocking shelves...");
        var localStock = generateLivestockStock();
        // Flatten to array for upload
        var allAnimals = [];
        Object.keys(localStock).forEach(function(sp){
          (localStock[sp]||[]).forEach(function(a){
            a.species = a.species || sp;
            allAnimals.push(a);
          });
        });
        window.baSupabaseSync.cleanOldMarketStock().then(function(){
          return window.baSupabaseSync.uploadGlobalMarketStock(allAnimals);
        }).then(function(){
          // Re-fetch to get the canonical data with proper IDs
          return window.baSupabaseSync.fetchGlobalMarketStock();
        }).then(function(result2){
          setStock(rowsToStock(result2.items));
          setLoading(false);
          setStatusMsg("");
        }).catch(function(err){
          console.warn("[BA Market] Upload failed, using local:", err);
          setStock(localStock);
          setLoading(false);
          setStatusMsg("");
        });
      ...
      console.warn("[BA Market] Global fetch failed, using local:", err);
      setStock(generateLivestockStock());
      setLoading(false);
      setStatusMsg("");
    });
  ...
  // Check for cycle change every 60s
  useState(function(){
    var t=setInterval(function(){
      if (window.baSupabaseSync && window.baSupabaseSync.getMarketCycleId) {
        var newCycle = window.baSupabaseSync.getMarketCycleId();
  ...
  // Build available breeds from current stock for dropdown
  var curStock = stock[species]||[];
  var availableBreeds = [];
  var seenBreeds = {};
  curStock.forEach(function(a){
    var bName = a.breed || "Unknown";
    if (!seenBreeds[bName]) {
      seenBreeds[bName] = true;
      availableBreeds.push({
        name: bName,
        type: a.type || a.group || "",
        group: a.group || a.type || ""
      });
    }
  });
  availableBreeds.sort(function(a,b){ return a.name.localeCompare(b.name); });

  // Sub-filter logic for cow (beef/dairy), goat (dairy/meat/fiber/dual), horse (by group)
  var hasSubFilter = (species==="cow"||species==="goat"||species==="horse"||species==="sheep"||species==="pig"||species==="duck"||species==="chicken");
  var subFilterOptions = [];
  if (species==="cow") subFilterOptions=["all","beef","dairy"];
  else if (species==="goat") subFilterOptions=["all","dairy","meat","fiber","dual"];
  else if (species==="sheep") subFilterOptions=["all","wool","meat","dual"];
  else if (species==="pig") subFilterOptions=["all","heritage","commercial","mini"];
  else if (species==="duck") subFilterOptions=["all","egg","meat","dual","ornamental"];
  else if (species==="chicken") subFilterOptions=["all","egg","meat","dual","ornamental"];
  else if (species==="horse") subFilterOptions=["all","Racing","Light","Sport","Western","Draft","Gaited","Pony"];

  var filteredBreeds = availableBreeds.filter(function(b){
    if (subFilter==="all") return true;
    if (species==="horse") return b.group===subFilter;
    return b.type===subFilter;
  });

  // Get price for species + breed context
  function getBuyPrice(speciesKey, breedInfo) {
    if (speciesKey==="horse") return HORSE_GROUP_PRICES[breedInfo.group]||1200;
    if (speciesKey==="cow") return breedInfo.type==="dairy"?600:500;
    if (speciesKey==="goat") return ({dairy:90,meat:120,fiber:100,dual:90})[breedInfo.type]||90;
    if (speciesKey==="sheep") return ({wool:200,meat:150,dual:175})[breedInfo.type]||150;
    if (speciesKey==="pig") return ({heritage:150,commercial:80,mini:200})[breedInfo.type]||100;
    if (speciesKey==="duck") return ({egg:25,meat:30,dual:28,ornamental:50})[breedInfo.type]||20;
    if (speciesKey==="chicken") return ({egg:15,meat:12,dual:18,ornamental:40})[breedInfo.type]||15;
    return LIVESTOCK_PRICES[speciesKey]||50;
  }

  // Find and buy an animal of the selected breed from stock
  function buyAnimal(sex) {
    if (!selBreed) return;
    var match = curStock.find(function(a){ return (a.breed||"")===selBreed && a.sex===sex; });
    if (!match) return;
    var breedInfo = availableBreeds.find(function(b){ return b.name===selBreed; })||{};
    var price = getBuyPrice(species, breedInfo);
    var animalToBuy = Object.assign({}, match, { price: price });
    var marketId = match._marketId || match.id;

    // Try Supabase delete first (claim the animal globally)
    if (window.baSupabaseSync && window.baSupabaseSync.buyGlobalMarketStock && marketId) {
      window.baSupabaseSync.buyGlobalMarketStock(marketId).then(function(success){
        if (success) {
          // We got it — complete the local buy
          var ok = onBuy(species, animalToBuy);
          if (ok) {
            setStock(function(prev){
              var nx=Object.assign({},prev);
              nx[species]=(prev[species]||[]).filter(function(a){return (a._marketId||a.id)!==marketId;});
              return nx;
            });
            setSess(function(s){return {bought:s.bought+1,bAmt:s.bAmt+price,sold:s.sold,sAmt:s.sAmt};});
          }
        } else {
          // Someone else got it first — refresh and notify
          alert("Sold out! Another player bought this animal. Refreshing stock...");
          loadGlobalMarket();
        }
      });
    } else {
      // No Supabase — local only fallback
      var ok = onBuy(species, animalToBuy);
      if (ok) {
        setStock(function(prev){
          var nx=Object.assign({},prev);
          nx[species]=(prev[species]||[]).filter(function(a){return a.id!==match.id;});
          return nx;
        });
        setSess(function(s){return {bought:s.bought+1,bAmt:s.bAmt+price,sold:s.sold,sAmt:s.sAmt};});
      }
  ...
  // Count available M/F for selected breed
  var maleCount = curStock.filter(function(a){return (a.breed||"")===selBreed && a.sex==="M";}).length;
  var femaleCount = curStock.filter(function(a){return (a.breed||"")===selBreed && a.sex==="F";}).length;
  var selBreedInfo = availableBreeds.find(function(b){return b.name===selBreed;})||{};
  var buyPrice = selBreed ? getBuyPrice(species, selBreedInfo) : 0;
  var canAfford = money >= buyPrice;
  ...
  // Calculate next refresh time from cycle ID
  var nextRefreshStr = "";
  if (currentCycle) {
    var parts = currentCycle.split("T");
    var cycleHour = parseInt(parts[1]||"0");
    var nextHour = cycleHour + 3;
    if (nextHour >= 24) nextHour = 0;
    // Convert UTC to local
    var utcDate = new Date(parts[0]+"T"+String(nextHour).padStart(2,"0")+":00:00Z");
    var pad=function(n){return String(n).padStart(2,"0");};
    nextRefreshStr = pad(utcDate.getHours())+":"+pad(utcDate.getMinutes());
  }
  ...
  var selDropStyle = { background:"#0f172a", border:"1px solid #334155", color:"#e2e8f0", borderRadius:6, padding:"5px 8px", fontSize:"0.8rem" };
  ...
  return /*#__PURE__*/React.createElement("div",{
    ...
    /*#__PURE__*/React.createElement("div",{
      ...
      mktTab==="buy"&&/*#__PURE__*/React.createElement("div",{
        ...
        LIVESTOCK_SPECIES.map(function(sp){
          return /*#__PURE__*/React.createElement("button",{key:sp.key,
            onClick:function(){setSpecies(sp.key);setSelBreed("");setSubFilter("all");},
      ...
      // Sub-filters (cow type, goat type, horse group)
      mktTab==="buy"&&hasSubFilter&&/*#__PURE__*/React.createElement("div",{
        ...
        /*#__PURE__*/React.createElement("span",{style:{color:"#64748b",fontSize:"0.75rem"}},
          species==="horse"?"Group:":"Type:"),
        subFilterOptions.map(function(f){
          var active=subFilter===f;
          var label=f==="all"?"All":f.charAt(0).toUpperCase()+f.slice(1);
          return /*#__PURE__*/React.createElement("button",{key:f,
            onClick:function(){setSubFilter(f);setSelBreed("");},
      ...
      /*#__PURE__*/React.createElement("div",{style:{flex:1,overflowY:"auto",padding:"12px 18px"}},
        mktTab==="buy"?(loading
          ?/*#__PURE__*/React.createElement("div",{style:{color:"#94a3b8",textAlign:"center",padding:40,fontSize:"0.9rem"}},
            statusMsg||"Loading market...")
          :/*#__PURE__*/React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:12}},
          // Info text
          /*#__PURE__*/React.createElement("div",{style:{color:"#64748b",fontSize:"0.78rem"}},
            "Select a breed and sex to purchase. Genetics are hidden until after purchase \u2014 choose wisely!"),
          // Breed dropdown + buy buttons row
          /*#__PURE__*/React.createElement("div",{style:{display:"flex",flexWrap:"wrap",gap:8,alignItems:"center"}},
            /*#__PURE__*/React.createElement("select",{
              value:selBreed,
              onChange:function(e){setSelBreed(e.target.value);},
              style:Object.assign({},selDropStyle,{minWidth:200})},
              /*#__PURE__*/React.createElement("option",{value:""},"\u2014 Select Breed \u2014"),
              filteredBreeds.map(function(b){
                var count=curStock.filter(function(a){return (a.breed||"")===b.name;}).length;
                var typeLabel=b.type?(b.group||b.type):"";
                return /*#__PURE__*/React.createElement("option",{key:b.name,value:b.name},
                  b.name+(typeLabel?" ("+typeLabel+")":"")+" \u2014 "+count+" available");
              })
            ),
            // Price display
            selBreed&&/*#__PURE__*/React.createElement("span",{style:{color:"#22c55e",fontWeight:"bold",fontSize:"0.85rem"}},
              "$"+buyPrice.toLocaleString()),
            // Male button
            /*#__PURE__*/React.createElement("button",{
              onClick:function(){buyAnimal("M");},
              disabled:!selBreed||!canAfford||maleCount<1,
              style:{background:selBreed&&canAfford&&maleCount>0?"#0f2d1e":"#1a1a1a",
                border:"1px solid "+(selBreed&&canAfford&&maleCount>0?"#38bdf8":"#334155"),
                color:selBreed&&canAfford&&maleCount>0?"#38bdf8":"#475569",
                borderRadius:6,padding:"5px 12px",cursor:selBreed&&canAfford&&maleCount>0?"pointer":"not-allowed",
                fontSize:"0.8rem",fontWeight:"bold"}},
              "\u2642 Male ("+maleCount+")"),
            // Female button
            /*#__PURE__*/React.createElement("button",{
              onClick:function(){buyAnimal("F");},
              disabled:!selBreed||!canAfford||femaleCount<1,
              style:{background:selBreed&&canAfford&&femaleCount>0?"#2d0f1e":"#1a1a1a",
                border:"1px solid "+(selBreed&&canAfford&&femaleCount>0?"#f472b6":"#334155"),
                color:selBreed&&canAfford&&femaleCount>0?"#f472b6":"#475569",
                borderRadius:6,padding:"5px 12px",cursor:selBreed&&canAfford&&femaleCount>0?"pointer":"not-allowed",
                fontSize:"0.8rem",fontWeight:"bold"}},
              "\u2640 Female ("+femaleCount+")")
          ),
          // Stock summary
          curStock.length===0&&/*#__PURE__*/React.createElement("div",{style:{color:"#475569",textAlign:"center",padding:30}},
            "No stock available. Check back after next refresh."),
          !selBreed&&curStock.length>0&&/*#__PURE__*/React.createElement("div",{style:{color:"#475569",fontSize:"0.78rem",textAlign:"center",padding:20}},
            "Pick a breed from the dropdown above to see available stock.")
        )):(
          mktTab==="sell"?(function(){
            var SELLABLE=[
              {key:"horse",  label:"Horses",   icon:"\uD83D\uDC0E",
                ...
                sellPrice:function(a){return Math.round((a.price||1000)*HORSE_RESALE_RATE);}},
              {key:"cow",    label:"Cows",     icon:"\uD83D\uDC04",
                ...
                sellPrice:function(a){return Math.round((a.type==="dairy"?600:500)*LIVESTOCK_RESALE_RATE);}},
              ...
              {key:"goat",   label:"Goats",    icon:"\uD83D\uDC10",
                ...
                buyPrice:function(a){return ({dairy:90,meat:120,fiber:100,dual:90})[a.type]||90;},
                sellPrice:function(a){return Math.round((({dairy:90,meat:120,fiber:100,dual:90})[a.type]||90)*LIVESTOCK_RESALE_RATE);}},
            ...
            return /*#__PURE__*/React.createElement("div",null,
              /*#__PURE__*/React.createElement("div",{style:{display:"flex",alignItems:"center",gap:10,marginBottom:12}},
                /*#__PURE__*/React.createElement("div",{style:{color:"#64748b",fontSize:"0.82rem"}},
                  species==="horse"?"Market pays 85% of purchase price.":"Market pays 50% of value. No cats ever."),
...
function runDailyIncomeTick(ownedLivestock, lastTickDate, facilitiesOwned) {
  ...
  // Male fertility now handled by prime age system
  ...
  var updatedAnimals = (ownedLivestock || []).map(function(animal) {
    ...
    if (animal.locked) return animal;
    if (animal.retiredLivestock) return animal;
    var animalAge = animal.ageMonths || 0;
    if (isDeclining(animalAge, species)) return animal;
    ...
    // Male fertility now part of prime age system
    ...
    if (inc.isCommodity) {
      ...
      if (species === "chicken" || species === "duck") {
        ...
        var ageMult = getProductionMultiplier(animalAge, species);
        dailyQty = Math.round(inc.baseDaily * yieldMult * storageMult * ageMult * 10) / 10;
        ...
        var ageMult = getProductionMultiplier(animalAge, species);
        dailyQty = Math.round(100 * yieldMult * storageMult * ageMult * 10) / 10;
        commodityGains["milk"] = (commodityGains["milk"] || 0) + dailyQty;
        bySpecies[species].commodity += dailyQty;

      } else if (species === "goat") {
        // Goat milk: dairy and dual does produce milk
        if ((animal.type === "dairy" || animal.type === "dual") && animal.sex === "F") {
          var goatMilk = animal.milkYield || (GOAT_MILK_OUTPUT[animal.type] || 0);
          if (goatMilk > 0) {
            var ageMult = getProductionMultiplier(animalAge, species);
            dailyQty = Math.round(goatMilk * yieldMult * storageMult * ageMult * 10) / 10;
            commodityGains["goat_milk"] = (commodityGains["goat_milk"] || 0) + dailyQty;
            bySpecies[species].commodity += dailyQty;
          }
        }
        // Fiber goats accumulate mohair (sheared via shearing shed, not auto-sold)
        // Fiber accumulation handled by shearing system, not daily income
      }
      ...
      var ageMult = getProductionMultiplier(animalAge, species);
      var dailyEarning = Math.round(inc.baseDaily * yieldMult * newMarketMult * ageMult * 100) / 100;
  ...
  // Retirement notices handled by longevity system in game.js
