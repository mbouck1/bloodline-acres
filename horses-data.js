...
function createHorse(breedName, sex, ageMonths) {
  ...
  return {
    ...
    lifespan: (function(){ var base = {Racing:300, Light:336, Sport:312, Western:348, Draft:300, Gaited:336, Pony:384}; var yrs = (base[breed.group]||336); return yrs + Math.round((Math.random()-0.5)*48); })()
  };
}
...
function calcHorseShowScore(horse, classKey) {
  ...
  if (horse.locked) return 0;
  if (typeof canShowByAge==="function"&&!canShowByAge(horse.ageMonths||0,"horse")) return 0;
...
function runHorseShow(horse, classKey, level) {
  ...
  if (!cls) return null;
  if (horse.locked) return { error:"Locked animals cannot compete. Unlock first." };
  if (typeof canShowByAge==="function"&&!canShowByAge(horse.ageMonths||0,"horse")) return { error:"This horse is past prime and can no longer compete in shows." };
...
function normalizeHorse(horse, existingNames) {
  ...
  return Object.assign({
    ...
    lifespan: horse.lifespan || (function(){ var base = {Racing:300, Light:336, Sport:312, Western:348, Draft:300, Gaited:336, Pony:384}; return (base[breedDef.group]||336) + Math.round((Math.random()-0.5)*48); })()
  }, horse, {
    genome: genome,
    coatColor: coatColor,
    healthScore: healthScore,
...
function HorseCard(props) {
  ...
  var horseAgeStatus = (typeof isDeclining==="function"&&isDeclining(horse.ageMonths||0,"horse")) ? "declining" : ((typeof isPastPrime==="function"&&isPastPrime(horse.ageMonths||0,"horse")) ? "past_prime" : "ok");
  ...
  return React.createElement("div",{
    ...
    React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4,gap:6}},
      editing
        ? React.createElement("div",{style:{display:"flex",gap:4,flex:1},onClick:function(e){e.stopPropagation();}},
            ...
            React.createElement("button",{onClick:function(e){e.stopPropagation();saveName();},
    ...
    React.createElement("div",{style:{display:"flex",gap:8,marginBottom:6,flexWrap:"wrap"}},
      ...
      horse.pregnantUntil && (function(){
        var daysLeft = Math.max(0, Math.ceil((horse.pregnantUntil - Date.now()) / (24*60*60*1000)));
        return React.createElement("span",{style:{color:"#f472b6",fontSize:"0.72rem",fontWeight:"bold"}},"🤰 "+daysLeft+"d");
      })()
    ),
    ...
    onLock && React.createElement("div",{style:{display:"flex",gap:4,marginTop:4}},
      React.createElement("button",{
        onClick:function(e){e.stopPropagation();onLock(horse);},
        style:{flex:1,background:horse.locked?"#1a1408":"#141008",
          border:"1px solid "+(horse.locked?"#f59e0b":"#4a3a28"),
          color:horse.locked?"#f59e0b":"#8a7055",
          borderRadius:6,padding:"4px 0",cursor:"pointer",fontSize:"0.72rem",fontWeight:"bold"}
      },horse.locked?"\uD83D\uDD13 Unlock":"\uD83D\uDD12 Lock")
    ),
    // Status badges
    (horse.locked||horseAgeStatus!=="ok") && React.createElement("div",{style:{display:"flex",gap:4,marginTop:4,flexWrap:"wrap"}},
      horse.locked && React.createElement("span",{style:{fontSize:"0.63rem",background:"#92400e",color:"#fef3c7",padding:"2px 8px",borderRadius:4,fontWeight:"bold"}},"\uD83D\uDD12 LOCKED"),
      !horse.locked && horseAgeStatus==="declining" && React.createElement("span",{style:{fontSize:"0.63rem",background:"#7f1d1d",color:"#fecaca",padding:"2px 8px",borderRadius:4,fontWeight:"bold"}},"\u26A0\uFE0F DECLINING"),
      !horse.locked && horseAgeStatus==="past_prime" && React.createElement("span",{style:{fontSize:"0.63rem",background:"#78350f",color:"#fde68a",padding:"2px 8px",borderRadius:4,fontWeight:"bold"}},"\u23F3 PAST PRIME")
    ),
...
function HorsesView(props) {
  ...
  var canBreed = hasStallion && hasMare;
  ...
  function ListRow(h) {
    ...
    return React.createElement("tr", {
      key: h.id,
      onClick: function(e){ e.stopPropagation(); setSelectedHorseId(selectedHorse&&selectedHorse.id===h.id?null:h.id); },
  ...
  return React.createElement("div",{
      ...
      React.createElement("div",{style:{display:"flex",gap:8,alignItems:"center"}},
        ...
        horses.length>=2 && React.createElement("button",{
          ...
          style:{background:canBreed?"#2a1e08":"#141008",border:"1px solid "+(canBreed?"#d4942a":"#2a2a18"),
            ...
            cursor:canBreed?"pointer":"not-allowed",fontSize:"0.8rem",fontWeight:"bold"}
        },"🤝 Breed"),
        React.createElement("button",{onClick:onClose,
    ...
    viewMode==="grid" && React.createElement("div",{style:{flex:1,overflow:"hidden",display:"flex",gap:0}},
      ...
      selectedHorse && React.createElement("div",{
        ...
        React.createElement(HorseCard,{horse:selectedHorse,onRename:onRename,onSell:onSell,onToggleStud:onToggleStud,onUpdateStudFee:onUpdateStudFee,onLock:props.onLock})
      )
    ),
    ...
    showBreeding && React.createElement(HorseBreedingModal,{
      ...
      onConfirm: function(sireId, damId){
        if (onBreed) onBreed(sireId, damId);
...
// Mendelian inheritance for a single diploid locus
function inheritLocus(sireAlleles, damAlleles) {
  var s = sireAlleles[Math.random() < 0.5 ? 0 : 1];
  var d = damAlleles[Math.random() < 0.5 ? 0 : 1];
  return [s, d];
}

// Blend two horse genomes into a foal genome
function breedHorseGenomes(sireGenome, damGenome) {
  var sc = sireGenome.coat || {}, dc = damGenome.coat || {};
  var sh = sireGenome.health || {}, dh = damGenome.health || {};
  var sp = sireGenome.perf || {}, dp = damGenome.perf || {};

  var coat = {};
  ["E","A","Cr","D","G","Rn","TO","Sv","Z","Ch"].forEach(function(loc) {
    coat[loc] = inheritLocus(sc[loc]||["0","0"], dc[loc]||["0","0"]);
  });

  var health = {};
  ["HipQ","BoneQ","LungQ","HeartQ","HoofQ"].forEach(function(loc) {
    health[loc] = inheritLocus(sh[loc]||["g","g"], dh[loc]||["g","g"]);
  });

  var perf = {};
  ["SPEED","STAMINA","MUSCLE","TEMP","AGILITY"].forEach(function(q) {
    var sa = sp[q]||[3,3], da = dp[q]||[3,3];
    var v0 = sa[Math.random()<0.5?0:1];
    var v1 = da[Math.random()<0.5?0:1];
    // Small mutation chance
    if (Math.random() < 0.005) v0 = Math.max(1, Math.min(5, v0 + (Math.random()<0.5?1:-1)));
    if (Math.random() < 0.005) v1 = Math.max(1, Math.min(5, v1 + (Math.random()<0.5?1:-1)));
    perf[q] = [v0, v1];
  });
...
// Create a foal object from sire + dam
function createFoal(sire, dam, foalNumber) {
  var genome = breedHorseGenomes(sire.genome, dam.genome);
  ...
  var perfScore = calcHorsePerfScore(genome);
  return {
    id: "horse_foal_" + Date.now() + "_" + Math.random().toString(36).substring(2,6),
    name: "Foal #" + foalNumber,
    breed: breed,
    group: breedDef.group || dam.group || "Western",
    species: "horse",
    type: dam.type || "light",
    sex: sex,
    ageMonths: 0,
    heightHands: breedDef.heightAvg || 14.0,
    weightLbs: Math.round((breedDef.weightAvg || 800) * 0.15),
    genome: genome,
    coatColor: coatColor,
    healthScore: healthScore,
    perfScore: perfScore,
    ...
    sireId: sire.id,
    damId: dam.id,
    price: Math.round(calcFoalValue(healthScore, perfScore) / 50) * 50,
    generation: Math.max(sire.generation||1, dam.generation||1) + 1,
    lifespan: (function(){ var sL = sire.lifespan || 336; var dL = dam.lifespan || 336; var avg = Math.round((sL+dL)/2); return avg + Math.round((Math.random()-0.5)*36); })()
  };
}

function calcFoalValue(healthScore, perfScore) {
  return 500 + Math.round((healthScore + perfScore) / 2 * 20);
}

// ── HORSE BREEDING MODAL ───────────────────────────────────────
function HorseBreedingModal(props) {
  var horses = props.horses || [];
  var onClose = props.onClose;
  var onConfirm = props.onConfirm;

  var allStallions = horses.filter(function(h){ return h.sex==="M" && !h.pregnantUntil && !h.locked && (typeof canBreedByAge!=="function"||canBreedByAge(h.ageMonths||0,"horse")); });
  var allMares     = horses.filter(function(h){ return h.sex==="F" && !h.pregnantUntil && !h.locked && (typeof canBreedByAge!=="function"||canBreedByAge(h.ageMonths||0,"horse")); });
